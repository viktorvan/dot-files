...
describe('State Transition Tests', () => {
  ...
  describe('USER_APPROVAL evidence validation', () => {
    test('user_approval_text with "yes" (lowercase) should pass', async () => {
      ...
      // USER_APPROVAL → DELEGATION with "yes" in lowercase
      const delegationResult = await client.callTool('request_next_state', {
        session_id: currentSessionId,
        evidence: {
    ...
    test('user_approval_text with "YES" (uppercase) should pass', async () => {
      // Create session and get to USER_APPROVAL state
      const currentSessionId = await createNewSession(client);
      ...
      // USER_APPROVAL → DELEGATION with "YES" in uppercase
      const delegationResult = await client.callTool('request_next_state', {
        session_id: currentSessionId,
        evidence: {
    ...
    test('user_approval_text with "Yes" (mixed case) should pass', async () => {
      // Create session and get to USER_APPROVAL state
      const currentSessionId = await createNewSession(client);
      ...
      // USER_APPROVAL → DELEGATION with "Yes" already tested in the main workflow test above
      // This test verifies it works in isolation too
      const delegationResult = await client.callTool('request_next_state', {
        session_id: currentSessionId,
        evidence: {
          user_approval_text: 'Yes, I approve this plan. Please proceed with implementation.'
    ...
    test('user_approval_text without "yes" should fail', async () => {
      // Create session and get to USER_APPROVAL state
      const currentSessionId = await createNewSession(client);
      ...
      // USER_APPROVAL → DELEGATION without "yes" - should fail
      await expect(client.callTool('request_next_state', {
        session_id: currentSessionId,
        evidence: {
          user_approval_text: 'I approve this plan completely.'
        }
      })).rejects.toThrow(/user did not explicitly approve.*yes.*ask for approval again/i);
    });
    ...
    test('user_approval_text with only "no" should fail', async () => {
      ...
      // USER_APPROVAL → DELEGATION with "no" instead of "yes" - should fail
      await expect(client.callTool('request_next_state', {
        session_id: currentSessionId,
        evidence: {
