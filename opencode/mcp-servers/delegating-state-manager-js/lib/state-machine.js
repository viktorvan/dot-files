...
export class StateMachine {
  constructor() {
    ...
    for (const [field, schema] of Object.entries(EVIDENCE_SCHEMAS)) {
      this.compiledSchemas[field] = this.ajv.compile(schema);
    }
  }

  /**
   * Get the next valid state for a current state
   * @param {string} currentState - Current state name
   * @returns {string|null} Next state name or null if terminal state
   * @throws {Error} If current state is invalid
   */
  ...
  /**
   * Get required evidence fields for a state
   * @param {string} state - State name
   * @returns {string[]} Array of required evidence field names
   * @throws {Error} If state is invalid
   */
  getRequiredEvidence(state) {
    if (!STATES[state]) {
      throw new Error(`Invalid state: ${state}. Valid states are: ${Object.keys(STATES).join(', ')}`);
    }

    return [...STATES[state].evidence]; // Return a copy to prevent mutation
  }

  /**
   * Validate a state transition
   * @param {string} currentState - Current state name
   * @param {string} nextState - Proposed next state name
   * @returns {boolean} True if transition is valid
   * @throws {Error} If transition is invalid with descriptive message
   */
  ...
  /**
   * Validate evidence completeness and format for a state
   * @param {string} state - State name
   * @param {Object} evidence - Evidence object to validate
   * @param {Object} session - Session object (for context)
   * @returns {boolean} True if evidence is valid
   * @throws {Error} If evidence is invalid with descriptive messages
   */
  validateEvidence(state, evidence, session) {
    if (!STATES[state]) {
      throw new Error(`Invalid state: ${state}. Valid states are: ${Object.keys(STATES).join(', ')}`);
    }
    ...
    for (const field of requiredFields) {
      ...
      if (this.compiledSchemas[field]) {
        ...
        if (!isValid) {
          ...
          const errorMessages = fieldErrors.map(error => {
      ...
      if (field === 'review_id' && session) {
        if (state === 'REVIEW_PLAN') {
          ...
          if (!session.implementation_review_id) {
            ...
            errors.push(`review_id does not match session implementation_review_id. Expected: ${session.implementation_review_id}, got: ${evidence[field]}`);
      ...
      if (field === 'task_ids' && session && state === 'DELEGATION') {
        const providedTaskIds = evidence[field] || [];
      ...
      // Validate user_approval_text contains "yes" for USER_APPROVAL state
      if (field === 'user_approval_text' && state === 'USER_APPROVAL') {
        const approvalText = evidence[field] || '';
        const containsYes = approvalText.toLowerCase().includes('yes');
        if (!containsYes) {
          errors.push(`The user did not explicitly approve the request with a 'yes', ask for approval again. Got: "${approvalText}"`);
        }
      }
    }
  ...
  /**
   * Check if a state is terminal (no next state)
   * @param {string} state - State name
